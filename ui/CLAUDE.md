...
- **Writes must use `useChainPinnedWrite()`** (`hooks/useChainPinnedWrite.ts`), never wagmi's `useWriteContract` directly. It is the same shape, but pins `chainId: activeChainId` onto every write. Without the pin wagmi submits on whatever chain the wallet is on and asserts nothing, so a wallet on another network sends the transaction there — to an address that only holds our contracts on the chain the UI is showing. A call to a codeless address succeeds, so any attached value is lost and the tx still reports confirmed. The hook exists so a new call site cannot reintroduce this by forgetting a field.
- **All user-facing writes go through `TxButton`** (which calls `useTx().track`) — don't call `writeContractAsync` directly for actions without tracking. `TxButton` is also where a write is refused when the wallet is on a different chain than the app (`hooks/useChainMismatch.ts`), so keep new write actions inside it rather than hand-rolling a button. Where a write genuinely cannot be a `TxButton` — `ServicesTab`'s restricted-rate pill and capability chips, which are inline row controls — call `useChainMismatch()` there and use it for both the `disabled` and the tooltip, or that write is the one the guard misses. `track` waits for the receipt, then `onConfirmed` fires and **all queries are invalidated** so the whole view refreshes. Use `onConfirmed` for side effects like clearing inputs; do **not** add manual refetch timers.
