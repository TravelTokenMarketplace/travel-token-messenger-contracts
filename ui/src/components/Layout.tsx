    ...
    <NavLink to={to} end={end} className={navLinkClass}>
      {({ isActive }) => (
        <span className="relative inline-block py-0.5">
          {children}
          {isActive && <span className="absolute -bottom-[3px] left-0 right-0 h-[2px] bg-brand-500" aria-hidden />}
        </span>
      )}
    </NavLink>
  );
        ...
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-x-6 gap-y-3 px-6 py-3">
          <div className="flex items-center gap-6">
            <Link to="/" className="group flex items-center gap-2">
              <span
                className="grid h-6 w-6 place-items-center rounded-sm bg-tarmac-900 font-display text-sm font-bold text-brand-400 dark:bg-brand-500 dark:text-tarmac-950"
                aria-hidden
              >
                ◆
              </span>
              <span className="font-display text-sm font-bold uppercase tracking-[0.12em] text-tarmac-900 dark:text-tarmac-50">
                Travel Token<span className="text-brand-600 dark:text-brand-400"> Messenger</span>
              </span>
            </Link>
