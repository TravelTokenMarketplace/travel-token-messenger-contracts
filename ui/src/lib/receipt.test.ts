...
const abi = parseAbi(["event TTMAccountCreated(address indexed account)"]);

describe("findCreatedAccount", () => {
  it("extracts the account address from logs", () => {
    const addr = "0x1111111111111111111111111111111111111111";
    // TTMAccountCreated has only an indexed arg, so the value lives in topics; data is empty.
    const topics = encodeEventTopics({ abi, eventName: "TTMAccountCreated", args: { account: addr } });
